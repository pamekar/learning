<?php
namespace AcademistCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'academist-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    array(
						    array(
							    ...
							    'value'      => array(
								    esc_html__( 'Tiny', 'academist-core' )   => 'eltdf-icon-tiny',
								    esc_html__( 'Small', 'academist-core' )  => 'eltdf-icon-small',
								    esc_html__( 'Medium', 'academist-core' ) => 'eltdf-icon-medium',
								    esc_html__( 'Large', 'academist-core' )  => 'eltdf-icon-large',
								    esc_html__( 'Huge', 'academist-core' )   => 'eltdf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'academist-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
							    ...
							    'value'      => array(
								    esc_html__( 'Normal', 'academist-core' ) => 'eltdf-normal',
								    esc_html__( 'Circle', 'academist-core' ) => 'eltdf-circle',
								    esc_html__( 'Square', 'academist-core' ) => 'eltdf-square'
							    ),
							    'save_always' => true
						    ),
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'academist-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'academist-core' ),
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'academist-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'academist-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'academist-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Icon Animation', 'academist-core' ),
							    'value'      => array_flip( academist_elated_get_yes_no_select_array( false ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'icon_animation_delay',
							    'heading'    => esc_html__( 'Icon Animation Delay (ms)', 'academist-core' ),
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'academist-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'academist-core' ),
							    'dependency'  => Array( 'element' => 'link', 'not_empty' => true )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'target',
							    'heading'    => esc_html__( 'Target', 'academist-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = academist_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'eltdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'eltdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . academist_elated_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'eltdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . academist_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . academist_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . academist_elated_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . academist_elated_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'eltdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . academist_elated_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
