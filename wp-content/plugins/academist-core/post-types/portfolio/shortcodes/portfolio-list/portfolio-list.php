...
class PortfolioList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Portfolio List', 'academist-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Portfolio List Template', 'academist-core' ),
							'value'       => array(
								esc_html__( 'Gallery', 'academist-core' ) => 'gallery',
						...
						array(
							...
							'heading'     => esc_html__( 'Number of Portfolios Per Page', 'academist-core' ),
							'description' => esc_html__( 'Set number of items for your portfolio list. Enter -1 to show all.', 'academist-core' ),
							'value'       => '-1'
						),
						array(
							'type'        => 'dropdown',
							...
							'description' => esc_html__( 'Set image proportions for your portfolio list.', 'academist-core' ),
							'dependency'  => array( 'element' => 'type', 'value' => array( 'gallery' ) )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'enable_fixed_proportions',
							'heading'     => esc_html__( 'Enable Fixed Image Proportions', 'academist-core' ),
							'value'       => array_flip( academist_elated_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Set predefined image proportions for your masonry portfolio list. This option will apply image proportions you set in Portfolio Single page - dimensions for masonry option.', 'academist-core' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Pagination Type', 'academist-core' ),
							'value'      => array(
								esc_html__( 'None', 'academist-core' )            => 'no-pagination',
								esc_html__( 'Standard', 'academist-core' )        => 'standard',
								esc_html__( 'Load More', 'academist-core' )       => 'load-more',
								esc_html__( 'Infinite Scroll', 'academist-core' ) => 'infinite-scroll'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Load More Top Margin (px or %)', 'academist-core' ),
						...
						array(
							'type'       => 'dropdown',
							'param_name' => 'filter',
							'heading'    => esc_html__( 'Enable Category Filter', 'academist-core' ),
							'value'      => array_flip( academist_elated_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Additional Features', 'academist-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Order By', 'academist-core' ),
							'value'      => array(
								esc_html__( 'Name', 'academist-core' )  => 'name',
								esc_html__( 'Count', 'academist-core' ) => 'count',
								esc_html__( 'Id', 'academist-core' )    => 'id',
								esc_html__( 'Slug', 'academist-core' )  => 'slug'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Filter Text Transform', 'academist-core' ),
							'value'      => array_flip( academist_elated_get_text_transform_array( true ) ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'academist-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Bottom Margin (px or %)', 'academist-core' ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'academist-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Enable Article Animation', 'academist-core' ),
							'value'       => array_flip( academist_elated_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Enabling this option you will enable appears animation for your portfolio list items', 'academist-core' ),
							'group'       => esc_html__( 'Additional Features', 'academist-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = academist_core_get_cpt_shortcode_module_template_part( 'portfolio', 'portfolio-list', 'portfolio-holder', $params['type'], $params, $additional_params );
		...
		if ( $params['type'] == 'masonry' && $params['enable_fixed_proportions'] === 'yes' ) {
			...
			switch ( $fixed_image_size ) {
				...
				case 'large-width-height':
					$thumb_size = 'academist_elated_image_huge';
					break;
				default :
					$thumb_size = 'full';
					break;
	...
	public function getStandardContentStyles( $params ) {
		...
		if ( ! empty( $margin_top ) ) {
			if ( academist_elated_string_ends_with( $margin_top, '%' ) || academist_elated_string_ends_with( $margin_top, 'px' ) ) {
		...
		if ( ! empty( $margin_bottom ) ) {
			if ( academist_elated_string_ends_with( $margin_bottom, '%' ) || academist_elated_string_ends_with( $margin_bottom, 'px' ) ) {
	...
	public function getFilterHolderStyles( $params ) {
		...
		if ( ! empty( $params['filter_bottom_margin'] ) ) {
			...
			if ( academist_elated_string_ends_with( $margin, '%' ) || academist_elated_string_ends_with( $margin, 'px' ) ) {
				...
				$styles[] = 'margin-bottom: ' . academist_elated_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getItemLink() {
		...
		return apply_filters( 'academist_elated_filter_portfolio_external_link', $portfolio_link );
	}
	...
	public function getItemLinkTarget() {
		...
		return apply_filters( 'academist_elated_filter_portfolio_external_link_target', $portfolio_link_target );
	}
