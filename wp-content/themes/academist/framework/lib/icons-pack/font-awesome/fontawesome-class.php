...
if ( ! function_exists( 'academist_elated_add_font_awesome_icon_pack' ) ) {
	function academist_elated_add_font_awesome_icon_pack( $iconCollections ) {
		$iconCollections['font_awesome'] = new AcademistElatedClassIconsFontAwesome( 'Font Awesome', 'fa_icon' );
		
		return $iconCollections;
	}
	
	add_filter( 'academist_elated_filter_add_icon_pack_into_collection', 'academist_elated_add_font_awesome_icon_pack' );
}

if ( ! function_exists( 'academist_elated_add_font_awesome_icon_pack_option' ) ) {
	function academist_elated_add_font_awesome_icon_pack_option( $options ) {
		$options['font_awesome'] = esc_html__( 'Font Awesome', 'academist' );
		
		return $options;
	}
	
	add_filter( 'academist_elated_filter_add_icon_pack_into_options', 'academist_elated_add_font_awesome_icon_pack_option' );
	add_filter( 'academist_elated_filter_add_icon_pack_into_social_options', 'academist_elated_add_font_awesome_icon_pack_option' );
}

class AcademistElatedClassIconsFontAwesome implements iAcademistElatedInterfaceIconCollection {
	public $icons;
	public $title;
	public $param;
	public $styleUrl;
	...
	function __construct($title = "", $param = "") {
		...
		$this->styleUrl = ELATED_FRAMEWORK_ICONS_ROOT . '/font-awesome/css/fontawesome-all.min.css';
	}
	...
	private function getNewIconsArray( $brandsIcons = false ) {
		...
		$icons = json_decode( $wp_filesystem->get_contents( ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/font-awesome/icons.json' ) );
	...
	public function render( $icon, $params = array() ) {
		...
		$html .= '<i class="eltdf-icon-font-awesome ' . $icon . ' ' . $iconClass . '" ' . $iconAttributesString . '></i>';
