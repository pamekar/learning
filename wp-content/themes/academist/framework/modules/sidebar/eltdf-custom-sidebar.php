 ...
 */
if ( ! class_exists( 'AcademistElatedClassSidebar' ) ) {
	class AcademistElatedClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'academist' );
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__('Enter Name of the new Widget Area', 'academist') . "' name='eltdf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__('Add Widget Area', 'academist') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area() {
			if ( ! empty( $_POST['eltdf-add-widget'] ) ) {
				...
				$name           = $this->get_name( $_POST['eltdf-add-widget'] );
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'academist_elated_filter_custom_widget_args', $args );
