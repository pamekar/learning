<?php
namespace AcademistCore\CPT\Shortcodes\BlogSlider;
...
class BlogSlider implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(
			array(
				'name'                      => esc_html__( 'Blog Slider', 'academist' ),
				'base'                      => $this->base,
				...
				'params'                    => array(
					array(
						...
						'value'       => array(
							esc_html__( 'Slider', 'academist' )            => 'slider',
							esc_html__( 'Carousel', 'academist' )          => 'carousel',
							esc_html__( 'Carousel Centered', 'academist' ) => 'carousel-centered'
						),
						'save_always' => true
					),
					array(
						...
						'heading'    => esc_html__( 'Number of Posts', 'academist' )
					),
					array(
						'type'        => 'dropdown',
						'param_name'  => 'orderby',
					...
					array(
						...
						'value'      => array(
							...
							esc_html__( 'Thumbnail', 'academist' ) => 'thumbnail',
					...
					array(
						...
						'group'       => esc_html__( 'Post Info', 'academist' )
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Author', 'academist' ),
					...
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Date', 'academist' ),
						'value'       => array_flip( academist_elated_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'academist' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Category', 'academist' ),
						'value'       => array_flip( academist_elated_get_yes_no_select_array( false, true ) ),
						'save_always' => true,
						'group'       => esc_html__( 'Post Info', 'academist' )
					),
					array(
						...
						'heading'     => esc_html__( 'Enable Post Info Comments', 'academist' ),
	...
	public function render( $atts, $content = null ) {
		...
		academist_elated_get_module_template_part( 'shortcodes/blog-slider/holder', 'blog', '', $params );
    ...
    public function blogListCategoryAutocompleteSuggester( $query ) {
        ...
        if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
            foreach ( $post_meta_infos as $value ) {
                ...
                $data['label'] = ( ( strlen( $value['category_title'] ) > 0 ) ? esc_html__( 'Category', 'academist' ) . ': ' . $value['category_title'] : '' );
                $results[]     = $data;
            }
        }
        ...
        if ( ! empty( $query ) ) {
            ...
            if ( is_object( $category ) ) {
                ...
                if ( ! empty( $category_title ) ) {
                    $category_title_display = esc_html__( 'Category', 'academist' ) . ': ' . $category_title;
                }
