...
if (!class_exists('AcademistElatedClassStickyNavigationWalker')) {
	class AcademistElatedClassStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide"){
					$menu_type = " wide";
				} else if( $item->type_menu == "wide-1-3" ) {
					$menu_type = " wide wide-1-3";
				} else {
					$menu_type = " narrow";
				}
			}
			...
			if($item->hide == ""){
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<i class="eltdf-menu-arrow arrow_carrot-down"></i>';
				}
